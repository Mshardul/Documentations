...
function renderQuestionCard(question, indexInFiltered) {
  ...
  question.options.forEach((opt, optIdx) => {
    ...
    optionsHtml += `
      ...
      question.id
    }-opt-${optIdx}" ${checkedAttr}>
                <label for="q-${question.id}-opt-${optIdx}">${escapeHtml(
      opt
    )}</label>
            </div>
        `;
  ...
  const tagsHtml = (question.tags || [])
    .map((tag) => {
      ...
      return `<span class="tag-chip" style="background-color: ${bgColor};">${escapeHtml(
        tag
      )}</span>`;
    })
    .join("");
  ...
  const answerHtml = `<div class="answer-section"><strong>Answer:</strong> ${escapeHtml(
  ...
  try {
    ...
    explanationHtml = `<div class="explanation-section"><strong>Explanation:</strong> ${escapeHtml(
      question.explanation || ""
    )}</div>`;
  }
  const answerAndExplanationHtml = state.showAnswer
    ? `${answerHtml}${explanationHtml}`
    : "";
...
function refreshCard(questionId) {
  ...
  if (!question) return;
  const indexInFiltered = filteredIndexes.findIndex(
    (idx) => allQuestions.questions[idx].id === questionId
  );
  ...
  radios.forEach((radio) => {
    radio.addEventListener("change", () => {
      ...
      if (!state.submitted) {
        ...
        questionStates.set(questionId, state);
        const clearBtnLocal = document
          .getElementById(`card-${questionId}`)
...
function renderQuestionsRange(start, end) {
  ...
  const questions = allQuestions.questions || [];
  console.log(
    `Rendering questions from index ${start} to ${end} (filteredIndexes length: ${filteredIndexes.length})`
  );
...
function loadMore() {
  console.log(
    "Load More clicked. Current loadedCount:",
    loadedCount,
    "Total filtered:",
    totalFilteredCount
  );
...
function updateActiveFiltersBar() {
  ...
  currentFilterTags.forEach((tag) => {
    const bgColor = getColorForTag(tag);
    chipsHtml += `<span class="active-filter-chip" style="background-color: ${bgColor};">${escapeHtml(
      tag
    )}</span>`;
  });
  // Add count badge
